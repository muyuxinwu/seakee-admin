...
class Configuration
{
	...
	{
		$items = [];
		if ($this->files->exists($file = $this->app->configFilePath())) {
	...
	public function save(RepositoryContract $config)
	{
		...
		$target = dirname($this->app->configFilePath());
		if (!$this->files->isDirectory($target)) {
			$this->files->makeDirectory($target, 0755, true);
		}

		// Save the configuration into the YAML file.
		$this->files->put($this->app->configFilePath(), $this->app->make(Dumper::class)->dump($config->all(), 10));
	}
