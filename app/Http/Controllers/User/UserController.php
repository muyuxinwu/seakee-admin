...
class UserController extends Controller
{
	...
	{
		return ['user_name' => 'required|max:255|unique:users', 'nick_name' => 'max:255|unique:users', 'email' => 'required|email|max:255|unique:users', 'phone' => 'regex:/^1[34578][0-9]{9}$/', 'password' => 'required|min:6|confirmed',];
	}
	...
	protected function errorInfo()
	{
		return [/**
		         * registration
		         */
		        'user_name.required' => '用户名不能为空', 'user_name.max' => '用户名不能超过255位', 'user_name.unique' => '用户名已存在', 'email.required' => 'email不能为空', 'email.max' => 'email不能超过255位', 'email.unique' => 'email已存在', 'email.email' => '请输入正确的email', 'password.required' => '密码不能为空', 'password.min' => '密码不能少于6位', 'password.confirmed' => '确认密码和密码不相符', 'nick_name.unique' => '昵称已存在', 'nick_name.max' => '昵称不能超过255位', 'phone.regex' => '手机号错误',];
	}
	...
	{
		return ['user_name' => 'required|max:255|unique:users,user_name,' . $id, 'nick_name' => 'max:255|unique:users,nick_name,' . $id, 'email' => 'required|email|max:255|unique:users,email,' . $id, 'phone' => 'regex:/^1[34578][0-9]{9}$/', 'password' => 'confirmed',];
	}
